		...
		Do
			...
			If Seq(SI + 1).Off = 0 Then
				...
				If BufferFull = True Then
					AddLitSequence()
					NewCloseBuffer()   'The whole literal sequence did not fit, buffer is full, close it
				End If
...
Err:
	...
	Public Function NewCloseBuffer() As Boolean
		On Error GoTo Err
...
Err:
		...
		MsgBox(ErrorToString(), vbOKOnly + vbExclamation, Reflection.MethodBase.GetCurrentMethod.Name + " Error")
NoDisk:
		NewCloseBuffer = False

	End Function

	Public Function NewCloseBundle(Optional NextFileIO As Integer = 0, Optional LastPartOnDisk As Boolean = False, Optional FromEditor As Boolean = False) As Boolean
		On Error GoTo Err
...
NewB:       'Next File Info does not fit, so close buffer
			NewCloseBuffer()               'Adds EndTag and starts new buffer
...
NoGo:
		NewCloseBundle = False

	End Function

	Public Sub NewCloseFile()
		On Error GoTo Err
