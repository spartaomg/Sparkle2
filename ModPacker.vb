...
#Const AllowShortMidMatches = False    'This would result in saving a few bytes per disk side but unpacking is slower...
    ...
    Private LastBlockOfBundle As Boolean = False
...
#If AllowShortMidMatches Then
                                        ...
                                        End If
#Else
                                    If (LL(Pos) < L) AndAlso (L > 2) Then 'Skip short (2-byte) Mid Matches
                                        LL(Pos) = L
                                        LO(Pos) = O
                                    End If
#End If
                        ...
                        If (LL(Pos) = MaxLL) AndAlso (SL(Pos) = MaxSL) Then
                            Exit For
                        End If
                    End If
                Next
...
Err:
    ...
    Private Sub CheckMatchSeq(SeqLen As Integer, SeqOff As Integer, Pos As Integer)
        ...
        For L As Integer = SeqLen To 2 Step -1
            'Calculate MatchBits
            MatchBits = If((L <= MaxShortLen) AndAlso (SeqOff <= MaxShortOffset), 8 + 1, If(L <= MaxMidLen, 16 + 1, 24 + 1))
...
Done:
        ...
        Dim BytesNeededForNextBundle As Integer = Int(BitsNeededForNextBundle / 8)
        If (LastBlockOfBundle) Then
            LastBlockOfBundle = False
            TransitionalBlock = True
            If (SequenceFits(BytesNeededForNextBundle, 0) = False) Then 'Bits=0, MLen will be checked in SequenceFits
...
Err:
    ...
    Private Sub CalcMatchBytesAndBits(Length As Integer, Offset As Integer) 'Match Length is 1 based
        ...
        If (Length <= MaxShortLen) AndAlso (Offset <= MaxShortOffset) Then
            MatchBytes = 1
...
Err:
    ...
    Private Function CalcLitBits(Lits As Integer) As Integer     'LitCnt is 0 based
        ...
        If (FirstLitOfBlock) AndAlso (CalcLitBits > 0) Then CalcLitBits -= 1
...
Err:
    ...
    Private Function SequenceFits(BytesToAdd As Integer, BitsToAdd As Integer, Optional SequenceUnderIO As Integer = 0) As Boolean
        ...
        If (TransitionalBlock) OrElse (BufferCnt = 0) Then
            BytesFree -= 1
        End If
        ...
        BytesToAdd += If((BlockUnderIO = 0) AndAlso (SequenceUnderIO = 1), 1, 0)
        ...
        If BytesFree >= BytesToAdd Then
            ...
            SequenceFits = True
            'Data will fit
            If (BlockUnderIO = 0) AndAlso (SequenceUnderIO = 1) Then
...
Err:
    ...
    Public Function CloseBuffer() As Boolean
        ...
        If (BitsLeftInBundle + BitsNeededForNextBundle + If(MLen = 0, 0, 1) + 8 <= ((LastByte - 1) * 8) + BitPos) AndAlso (LastFileOfBundle = True) AndAlso (NewBlock = False) Then
        ...
        If (LastBlockOfBundle) OrElse (NewTrack) OrElse (BlockCnt = 1) Then
            ...
            If (BlockCnt <> 1) AndAlso (NewTrack = False) Then  'Do not recalculate the very first block and first blocks on each track
                'Last/Transitional block
                BitsLeftInBundle = Seq(SI + 1).TotalBits + ((Seq(SI + 1).Nibbles Mod 2) * 4)
                'If the new bit count does not fit in the buffer then this is NOT the last block -> recalc sequence
                'KARAOKE BUG - fixed in Sparkle 2.2
                If BitsLeftInBundle + BitsNeededForNextBundle + If(MLen = 0, 0, 1) + 8 > ((LastByte - 1) * 8) + BitPos Then
                    LastBlockOfBundle = False
                    GoTo CalcAll
                End If
            End If
...
NoDisk:
    ...
    Public Function CloseBundle(Optional NextFileIO As Integer = 0, Optional LastPartOnDisk As Boolean = False, Optional FromEditor As Boolean = False) As Boolean
        ...
        If SequenceFits(Bytes, Bits) Then       'This will add the EndTag to the needed bytes
            ...
            If (MLen > 0) OrElse (LitCnt = -1) Then AddBits(1, 1)
...
NoGo:
    ...
    Public Sub CloseFile()
        ...
        Dim BytesNeededForNextFile As Integer = 4 + CheckIO(PrgLen - 1)
        ...
        If SequenceFits(BytesNeededForNextFile, BitsNeededForNextFile) Then   'DO NOT INCLUDE NEXT NEXT FILE'S IO STATUS HERE - IT WOULD RESULT IN AN UNWANTED I/O FLAG INSERTION

            'Buffer has enough space for New File Match Tag and New File Info and first Literal byte (and I/O flag if needed)

            'If last sequence was a match (no literals) then add a match bit
            If (MLen > 0) OrElse (LitCnt = -1) Then AddBits(MatchSelector, 1)
...
Err:
    ...
    Public Function CheckIO(Offset As Integer, Optional NextFileUnderIO As Integer = -1) As Integer
        ...
        ElseIf NextFileUnderIO > -1 Then
            CheckIO = If((Offset >= &HD000) AndAlso (Offset <= &HDFFF) AndAlso (NextFileUnderIO = 1), 1, 0)
        Else
            CheckIO = If((Offset >= &HD000) AndAlso (Offset <= &HDFFF) AndAlso (FileUnderIO = True), 1, 0)
        End If
