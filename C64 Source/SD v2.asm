...
Zone3:	pha			//+3	php? 						   65
		pla			//+4	plp?						   68
Zone2:	sta	(FakeAdd),y	//+6	($a0)=$fc4b+Y (ROM)		  67	   74

Zone1:	nop 			//+2					 63	  69	   76

Zone0:	eor	$0102,x	//$01ff^...			61	 67	  73	   80
...
LoopMod4:	ldx	ZP0f		// <-> ldx #$0f	  2/3	74	 81	  87	   94
		sax.z	t5+1		//t5+1 = 00005555	  3	77
		arr	#$f0		//A=44444000	  2	79
		tay			//Y=44444000	  2	81

LoopMod5:				//			     [78-103  84-111  90-119  96-127]
Read4:	lda	$1c01-$0f,x	//Read4 = 56666677  4/5	85/+7   93/+9   99/+9   106/+10
					//<->lda $1c01
		sax.z	t7+1		//t7+1 = 00006677	  3	88
FakeAdd:	alr	#$fc		//A=05666660, C=0	  2	90
		tax			//X=05666660	  2	92
...
t6:		lda	Tab6,x	//00000000,05666660	113
...
Read1:	lda	$1c01		//Read1 = 11111222  4	19/-6  19/-8  19/-10 19/-12
LoopMod1:	ldx	ZP07		// <-> ldx #$07	  2/3	21	 22	  22	   22
		sax.z	t2+1		//t2+1=00000222	  3	24	 25	  25	   25
LoopMod2:	and	ZPf8-$07,x	// <-> and #$f8	  2/4	26	 29	  29	   29
		tay			//Y=11111000	  2	28	 31	  31	   31

LoopMod3:	ldx	ZP3e		// <-> ldx #$3e	  2/3	30	 34	  34	   34
					//			     [26-51  28-55  30-59  32-63]
Read2:	lda	$1c01		//A=22333334	  4	34/+8  38/+10 38/+8  38/+6
...
GCREntry:
ModGCR:	bne	Zone1		//				57/56	 61/60  61/60  61/60
...
//-------------------------------------------------------------------------------------
		...
		tax			//Save checksum in X	66	 70	  70	   70
					//			     [52-77  56-83  60-89  64-95]
		lda	$1c01		//Final read = 44445555	70/-7  74/-9  74/+14 74/+10
		arr	#$f0		//A=44444000		72	 
		tay			//Y=44444000		74
		txa			//Return checksum to A	76
		ldx	t3+1		//X=00333330		79
		eor	Tab3,x	//(ZP)			83
		eor	Tab4,y	//Checksum (Data) or ID1 (Header) 		87
		eor	CSum+1	//Calculate final checksum	    		90
		bne	FetchAgain	//If A=#$00 here then checksum is OK	92	96: 56666677
ModJmp:	jmp	(HeaderJmp)	//Continue if A=#$00 (Checksum OK)		97	101	101	101
